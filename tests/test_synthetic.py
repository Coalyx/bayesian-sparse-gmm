...
def test_synthetic_sparse_recovery():
    ...
    for k in range(K_true):
        mask = true_labels == k
        X[mask, :n_signal] += rng.normal(loc=rng.choice([-5, 5]), scale=0.5, size=(mask.sum(), n_signal))
    ...
    assert recall >= 0.8, f"Expected to recover most signal features, got recall={recall:.3f}"
    assert precision > 0.5, f"Expected high precision on selected features, got precision={precision:.3f}"
