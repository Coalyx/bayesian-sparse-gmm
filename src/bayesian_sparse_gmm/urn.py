...
def truncated_poisson_pmf(K: int, lambda_pois: float, K_max: int) -> float:
    ...
    if K < 1 or K > K_max:
        return 0.0
    log_probs = np.arange(1, K_max + 1) * np.log(lambda_pois) - gammaln(
        np.arange(1, K_max + 1) + 1.0
    )
...
def log_urn_weight(
    k: int, n: int, alpha: float, lambda_pois: float, K_max: int
) -> float:
    ...
    log_p_K_unnorm = np.arange(1, K_max + 1) * np.log(lambda_pois) - gammaln(
        np.arange(1, K_max + 1) + 1.0
    )
