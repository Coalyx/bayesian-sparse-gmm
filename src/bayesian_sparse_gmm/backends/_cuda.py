...
class CUDABackend(ComputeBackend):
    ...
    def __init__(self):
        if not CUPY_AVAILABLE:
            raise ImportError("CuPy is not installed. Please install cupy to use CUDABackend.")
    ...
    def sample_inverse_gaussian(
        ...
        term = cp.sqrt(cp.maximum(0.0, 4.0 * inv_mean * shape * y_gpu + inv_mean2 * (y_gpu ** 2)))
        x1 = inv_mean / (1.0 + (inv_mean * y_gpu) / (2.0 * shape) + term / (2.0 * shape))
