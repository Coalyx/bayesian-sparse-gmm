...
from ._cuda import CUDABackend, CUPY_AVAILABLE
...
def select_backend(preference: str = "auto", use_cuda: bool = False) -> ComputeBackend:
    ...
    if preference == "cuda":
        ...
        warnings.warn("CUDA backend requested but neither CuPy nor Numba CUDA is available. Falling back to NumPy.")
        return NumpyBackend()
    ...
    if preference in ("numba", "auto"):
        if preference == "numba":
            ...
            warnings.warn("Numba backend requested but Numba is not available. Falling back to NumPy.")
            return NumpyBackend()
